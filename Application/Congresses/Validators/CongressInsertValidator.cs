...
public class CongressInsertValidator : AbstractValidator<CongressInsertDto>
{
    public CongressInsertValidator()
    {
        RuleFor(x => x.Name).NotEmpty().WithMessage("Nombre no puede ser vacío");
        RuleFor(x => x.Name).Length(2, 50).WithMessage("El nombre debe tener entre 2 y 50 caracteres.");
